...
extern "C" bool runLinearQ41_L3(queue &   q,
                                unsigned  m,
                                unsigned  n,
                                unsigned  k,
                                uint8_t * a,
...
ESIMD_INLINE void slimGemmGroup32Block256BatchNRangeN_L3(uint8_t *    a,
                                                         uint8_t *    b,
                                                         uint8_t *    s,
                                                         uint8_t *    z,
                                                         uint8_t *    c,
...
#pragma unroll
    for (int k = 0; k < 4; k++) {
        bb.template bit_cast_view<unsigned char>().template select<256, 1>(256 * k) =
            __ESIMD_ENS::lsc_block_load<uint8_t,
                                        256,
    ...
    for (int n = 0; n < pixelPerGroup; n += 2) {
        quant.select<16, 1>(0) = __ESIMD_ENS::lsc_gather<fp16,
                                                         1,
        ...
        zps.select<16, 1>(0) = __ESIMD_ENS::lsc_gather<fp16,
                                                       1,
                                                       __ESIMD_ENS::lsc_data_size::u16,
                                                       __ESIMD_ENS::cache_hint::cached,
                                                       __ESIMD_ENS::cache_hint::cached,
...
#pragma unroll
        for (int k = 0; k < localRange; k++) {
            bb.select<pixelPerGroup, 1>(0) += slm_block_load<float, pixelPerGroup>(
                pixelPerGroup * k * sizeof(float) + hh * localRange * pixelPerGroup * sizeof(float));
        }

        __ESIMD_ENS::lsc_block_store<float,
                                     pixelPerGroup,
                                     __ESIMD_ENS::lsc_data_size::default_size,
                                     __ESIMD_ENS::cache_hint::write_back,
                                     __ESIMD_ENS::cache_hint::write_back>((float *) c + outputOffset + hh * rowSize,
                                                                          bb.select<pixelPerGroup, 1>(0));
    }
}

template <uint32_t batchnum>
ESIMD_INLINE void slimGemmGroup32Block256BatchN_L3(uint8_t *    a,
                                                   uint8_t *    b,
                                                   uint8_t *    s,
                                                   uint8_t *    z,
                                                   uint8_t *    c,
        ...
        if (hh * 64 + k * localRange * 64 < commonDim) {
            bb.template bit_cast_view<unsigned char>().template select<256, 1>(256 * k) =
                __ESIMD_ENS::lsc_block_load<uint8_t,
                                            ...
                                            __ESIMD_ENS::cache_hint::cached,
                                            __ESIMD_ENS::cache_hint::cached>(
                    (uint8_t *) b + (hh * 64 + k * localRange * 64) * sizeof(float));
        } else {
...
#pragma unroll
            for (int k = 0; k < localRange; k++) {
                bb.select<pixelPerGroup, 1>(0) += slm_block_load<float, pixelPerGroup>(
                    pixelPerGroup * k * sizeof(float) + t_idx * localRange * pixelPerGroup * sizeof(float));
            }
            ...
            __ESIMD_ENS::lsc_block_store<float,
                                         ...
                                         __ESIMD_ENS::cache_hint::write_back,
                                         __ESIMD_ENS::cache_hint::write_back>(
                (float *) c + outputOffset + t_idx * rowSize, bb.select<pixelPerGroup, 1>(0));
        }
    }
}

template <uint32_t localRange>
static inline sycl::event executeSlimRunLinearBlock256RangeN_L3(queue &   q,
                                                                uint8_t * a,
                                                                uint8_t * b,
                                                                uint8_t * s,
                                                                uint8_t * z,
    ...
    if (k == 1) {
        e = q.submit([&](handler & cgh) {
            cgh.parallel_for(RangeV2, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL {
                slimGemmGroup32Block256BatchNRangeN_L3<1, localRange>(
                    (uint8_t *) a, (uint8_t *) b, (uint8_t *) s, (uint8_t *) z, (uint8_t *) c, m, ndi);
            });
        });
    } else {
...
static inline sycl::event executeSlimRunLinearBlock256_L3(queue &   q,
                                                          uint8_t * a,
                                                          uint8_t * b,
                                                          uint8_t * s,
                                                          uint8_t * z,
    ...
    if (k == 1) {
        e = q.submit([&](handler & cgh) {
            cgh.parallel_for(RangeV2, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL {
                slimGemmGroup32Block256BatchN_L3<1>(
                    (uint8_t *) a, (uint8_t *) b, (uint8_t *) s, (uint8_t *) z, (uint8_t *) c, n, m, ndi);
            });
        });
    } else {
        printf("Error! Q41 GEMV only supports k = 1 right now.\n");
...
bool runGemv_Q41Weights_L3(sycl::queue * q,
                           uint8_t *     inputs,
                           uint8_t *     weights,
                           uint8_t *     scales,
                           uint8_t *     zps,
