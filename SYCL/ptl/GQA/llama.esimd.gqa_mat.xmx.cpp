...
extern "C" bool runGQA_mat_fusion_xmx_simd16(sycl::queue* q, uint8_t* query, uint8_t* kCache, uint8_t* vCache, uint8_t* outputs, uint32_t token_len, uint32_t kv_len, uint32_t kv_head, uint32_t q_head, uint32_t q_precision, uint32_t o_precision, float attn_scale, uint8_t* shuffleTt);
extern "C" bool runGQA_mat_fusion_xmx_simd8(sycl::queue* q, uint8_t* query, uint8_t* kCache, uint8_t* vCache, uint8_t* outputs, uint32_t token_len, uint32_t kv_len, uint32_t kv_head, uint32_t q_head, float attn_scale, uint8_t* shuffleTt);

ESIMD_INLINE void shuffle_KCache(fp16* key, fp16* shuffleKey, int kv_len, int kv_head, nd_item<2>& ndi)
{
  const size_t head_idx = ndi.get_global_id(0);
  const size_t block_idx = ndi.get_global_id(1);

  uint32_t alignedKVLen = (kv_len + 15)/16*16;

  const fp16 * key_block = key + head_idx * 128 + block_idx * 8 * kv_head * 128;
  fp16 * new_key_block = shuffleKey + head_idx * alignedKVLen * 128 + block_idx * 8 * 128;

  simd<fp16, 1024> shuffleKBlock = 0.0;
#pragma unorll
  for (int i = 0; i < 8; ++i)
  {
      if (block_idx * 8 + i < kv_len)
      {
          simd<fp16, 128> key_row = block_load<fp16, 128>(key_block + i * kv_head * 128);
          shuffleKBlock.template bit_cast_view<float>().template select<64, 8>(i) = key_row.template bit_cast_view<float>();
      }
  }

  block_store<fp16, 1024>(new_key_block, shuffleKBlock);
}

ESIMD_INLINE void shuffle_VCache(fp16* value, fp16* shuffleValue, int kv_len, int kv_head, nd_item<2>& ndi)
{
  const size_t head_idx = ndi.get_group(0);
  const size_t block_idx = ndi.get_group(1);
  const size_t block_x = block_idx & 0x01;
  const size_t block_y = block_idx >> 1;

  uint32_t alignedKVLen = (kv_len + 15)/16*16;

  const fp16 * value_block = value + head_idx * 128 + block_y * 16 * kv_head * 128 + block_x * 64;
  fp16 * new_value_block = shuffleValue + head_idx * alignedKVLen * 128 + block_y * 16 * 128 + block_x * 16 * 64;

  simd<fp16, 1024> shuffleVBlock = 0.0;
#pragma unroll
  for (int i = 0; i < 16; i ++)
  {
      if (block_y * 16 + i < kv_len)
      {
          simd<fp16, 64> vTemp = block_load<fp16, 64>(value_block + i * kv_head * 128);
          int xx = i &0x01;
          int yy = i >> 1;
#pragma unroll
          for (int j = 0; j < 8; j++)
          {
              shuffleVBlock.select<8, 2>(j * 16 * 8 + yy * 2 * 8 + xx) = vTemp.select<8, 1>(8 * j);
          }
      }
  }

  block_store<fp16, 1024>(new_value_block, shuffleVBlock);
}


ESIMD_INLINE void gqa_mat_kernel_hidden128_simd8(uint8_t* qState, uint8_t* kState, uint8_t* vState, uint8_t* out, int token_len, int kv_len, uint32_t KVHead, uint32_t QHead, float attn_scale, nd_item<2>& ndi)
{
...
#pragma unroll
        ...
        bool needCompute = 32*v + (hh + 1) * 8 + startPos >=  l * loopStep + 8 * vv;
...
#pragma unroll
        ...
        qkrow.merge(FP32_MIN, k_idx > t_idx);
...
ESIMD_INLINE void gqa_mat_kernel_hidden128_simd16(uint8_t* qState, uint8_t* kState, uint8_t* vState, uint8_t* out, int token_len, int kv_len, uint32_t KVHead, uint32_t QHead, float attn_scale, nd_item<2>& ndi)
{
    ...
    {
        qDataBuf0 = block_load<IT, 128>((IT*)qState + offsetQ + localLinearId * QHead * 128);
    }
    if (v * 64 + localLinearId + 32 < token_len)
    {
        qDataBuf1 = block_load<IT, 128>((IT*)qState + offsetQ + (32 + localLinearId) * QHead * 128);
    }
...
#pragma unroll
        ...
        qkrow0.merge(FP32_MIN, k_idx > t_idx);
        qkrow1.merge(FP32_MIN, k_idx > t_idx + 1);
...
#pragma unroll
    for (int i = 0; i < 8; i++)
    {
        if (v * 64 + hh * 8 + i < token_len)
        {
            ...
            block_store<OT, 32>((OT*)out + outputOffset + (hh * 8 + i) * QHead * 128 + vv * 32, temp);
        }
    }
...
bool runGQA_mat_fusion_xmx_simd16(sycl::queue* q, uint8_t* query, uint8_t* kCache, uint8_t* vCache, uint8_t* outputs, uint32_t token_len, uint32_t kv_len, uint32_t kv_head, uint32_t q_head, uint32_t q_precision, uint32_t o_precision, float attn_scale, uint8_t* shuffleTt) {
    sycl::event e;

    // const uint32_t SimdWidth = 8;
    // const uint32_t alignedKVLen = (kv_len + 15) / 16 * 16;
    try {
        // uint8_t* shuffledKCache = shuffleTt;
        // uint8_t* shuffledVCache = shuffleTt + alignedKVLen * kv_head * 128 * sizeof(fp16);
        // Shuffle K
        // {
        //     sycl::range<2> GlobalRange(kv_head, alignedKVLen / SimdWidth);
        ...
        //             shuffle_KCache((fp16*)kCache, (fp16*)shuffledKCache, kv_len, kv_head, ndi);
        //           });
        //         });
        // }
        // Shuffle V
        // {
        //     sycl::range<2> GlobalRange(kv_head, alignedKVLen / 16 * 2 );
        //     sycl::range<2> LocalRange(1, 1);
        //     sycl::nd_range<2> Range(GlobalRange, LocalRange);
        //     e = q->submit([&](handler& cgh) {
        //         cgh.parallel_for(Range, [=](nd_item<2> ndi) SYCL_ESIMD_KERNEL{
        //             shuffle_VCache((fp16*)vCache, (fp16*)shuffledVCache, kv_len, kv_head, ndi);
        //           });
        //         });
        // }

        // q->wait();
        // FILE* fp = fopen("shuffled_v_cache.bin", "wb");
        // uint8_t* temp = new uint8_t[kv_len * kv_head * 128 * sizeof(fp16)];
        // q->memcpy(temp, shuffledVCache, kv_len * kv_head * 128 * sizeof(fp16)).wait();
        // fwrite(temp, 1, kv_len * kv_head * 128 * sizeof(fp16), fp);
        // delete[] temp;
        // fclose(fp);
        ...
        // GQA
        {
            ...
            if (q_precision == 0 && o_precision == 0)
            {
                e = q->submit([&](handler& cgh) {
                    cgh.parallel_for(Range, [=](nd_item<2> ndi) SYCL_ESIMD_KERNEL{
                        gqa_mat_kernel_hidden128_simd16<float, float>(query, kCache, vCache, outputs, token_len, kv_len, kv_head, q_head, attn_scale, ndi);
                    });
                });
            }
            else if (q_precision == 1 && o_precision == 0)
            {
                e = q->submit([&](handler& cgh) {
                    cgh.parallel_for(Range, [=](nd_item<2> ndi) SYCL_ESIMD_KERNEL{
                        gqa_mat_kernel_hidden128_simd16<fp16, float>(query, kCache, vCache, outputs, token_len, kv_len, kv_head, q_head, attn_scale, ndi);
                    });
                });
            }
            else if (q_precision == 0 && o_precision == 1)
            {
                e = q->submit([&](handler& cgh) {
                    cgh.parallel_for(Range, [=](nd_item<2> ndi) SYCL_ESIMD_KERNEL{
                        gqa_mat_kernel_hidden128_simd16<float, fp16>(query, kCache, vCache, outputs, token_len, kv_len, kv_head, q_head, attn_scale, ndi);
                    });
                });
            }
            else if (q_precision == 1 && o_precision == 1)
            {
                e = q->submit([&](handler& cgh) {
                    cgh.parallel_for(Range, [=](nd_item<2> ndi) SYCL_ESIMD_KERNEL{
                        gqa_mat_kernel_hidden128_simd16<fp16, fp16>(query, kCache, vCache, outputs, token_len, kv_len, kv_head, q_head, attn_scale, ndi);
                    });
                });
            }
        }
...
bool runGQA_mat_fusion_xmx_simd8(sycl::queue* q, uint8_t* query, uint8_t* kCache, uint8_t* vCache, uint8_t* outputs, uint32_t token_len, uint32_t kv_len, uint32_t kv_head, uint32_t q_head, float attn_scale, uint8_t* shuffleTt) {
    sycl::event e;
    ...
    try {
        ...
        // GQA
        {
            ...
            e = q->submit([&](handler& cgh) {
                cgh.parallel_for(Range, [=](nd_item<2> ndi) SYCL_ESIMD_KERNEL{
                    gqa_mat_kernel_hidden128_simd8(query, kCache, vCache, outputs, token_len, kv_len, kv_head, q_head, attn_scale, ndi);
                  });
                });
        }
    } catch (sycl::exception const &e) {
