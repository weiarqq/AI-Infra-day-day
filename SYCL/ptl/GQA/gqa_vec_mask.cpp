...
#pragma unroll
    for (int kk = 0; kk < 8; kk++) {
        qqFp32.template bit_cast_view<unsigned char>().template select<256, 1>(256 * kk) = __ESIMD_ENS::lsc_block_load<
            unsigned char,
            256,
    ...
    for (int loopIdx = 0; loopIdx < kvSeqOutLoopCount; loopIdx++) {
        vvState.template bit_cast_view<uint32_t>().template select<64, 1>(0) = __ESIMD_ENS::lsc_gather<
            uint32_t,
            4,
            ...
            __ESIMD_ENS::cache_hint::cached,
            16,
            uint32_t>((uint32_t*)vState, offsetV);

        if (kvSeqOffset < kvSeqLen) {
            kk.template bit_cast_view<unsigned char>().template select<256, 1>(0) = __ESIMD_ENS::lsc_block_load<
                unsigned char,
                256,
                __ESIMD_ENS::lsc_data_size::default_size,
                __ESIMD_ENS::cache_hint::cached,
...
ESIMD_INLINE void gqa_kernel_hidden128(uint8_t* qState, uint8_t* kState, uint8_t* vState, uint8_t* out, int kv_len, uint32_t KVHead, uint32_t QHead, float attn_scale, nd_item<2>& ndi)
{
    ...
    // Load Q data
    qData.select<64, 1>(0) = __ESIMD_ENS::lsc_block_load<
        IT,
        64,
        __ESIMD_ENS::lsc_data_size::default_size,
        __ESIMD_ENS::cache_hint::cached,
        __ESIMD_ENS::cache_hint::cached>((IT*)qState + offsetQ);
    qData.select<64, 1>(64) = __ESIMD_ENS::lsc_block_load<
        IT,
        64,
        __ESIMD_ENS::lsc_data_size::default_size,
        __ESIMD_ENS::cache_hint::cached,
...
#pragma unroll
        for (int s = 0; s < Step; s++) {
            if (l * loopStep + localLinearId * Step + s < kv_len - token_len + t + 1) {
                kData.template select<128, 1>(s * 128) = __ESIMD_ENS::lsc_block_load<
                    fp16,
                    128,
                    __ESIMD_ENS::lsc_data_size::default_size,
                    __ESIMD_ENS::cache_hint::cached,
...
#pragma unroll
        for (int s = 0; s < Step; s++) {
            if (l * loopStep + localLinearId * Step + s < kv_len - token_len + t + 1) {
                vData.template select<128, 1>(s * 128) = __ESIMD_ENS::lsc_block_load<
                    fp16,
                    128,
                    __ESIMD_ENS::lsc_data_size::default_size,
                    __ESIMD_ENS::cache_hint::cached,
...
#pragma unroll
    ...
    qqFp32.select<64, 1>(0) = __ESIMD_ENS::lsc_gather<
        float,
        4,
        __ESIMD_ENS::lsc_data_size::u32,
        __ESIMD_ENS::cache_hint::cached,
        __ESIMD_ENS::cache_hint::cached,
        16,
        uint32_t>((float*)qState, gatherOffsetQ);
    ...
    for (int loopIdx = 0; loopIdx < kvSeqOutLoopCount; loopIdx++) {
        ...
        vvState.select<128, 1>(0) = __ESIMD_ENS::lsc_block_load<
            fp16,
            128,
            __ESIMD_ENS::lsc_data_size::default_size,
            __ESIMD_ENS::cache_hint::cached,
        ...
        vvState.select<128, 1>(128) = __ESIMD_ENS::lsc_block_load<
            fp16,
            128,
            __ESIMD_ENS::lsc_data_size::default_size,
            __ESIMD_ENS::cache_hint::cached,
        ...
        if (kvSeqOffset < kvSeqLen) {
            kk.select<128, 1>(0) = __ESIMD_ENS::lsc_block_load<
                fp16,
                128,
                __ESIMD_ENS::lsc_data_size::default_size,
                __ESIMD_ENS::cache_hint::cached,
        ...
        if (kvSeqOffset + 32 < kvSeqLen) {
            kk.select<128, 1>(128) = __ESIMD_ENS::lsc_block_load<
                fp16,
                128,
                __ESIMD_ENS::lsc_data_size::default_size,
                __ESIMD_ENS::cache_hint::cached,
