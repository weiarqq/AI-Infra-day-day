...
extern "C" void __declspec(dllexport) RunMhaQ80Arb_xve(void* stream, const void* query, uint8_t* kCache, uint8_t* vCache, uint8_t* mask, void* outputs, uint32_t token_len, uint32_t kv_len, uint32_t kv_head, uint32_t q_head, uint32_t head_dim, float attn_scale, int input_precision, int output_precision);

// MT: mask element type (float for fp32 mask, fp16 for fp16 mask)
template <typename IT, typename OT, int HEAD_DIM>
void RunVecMhaQ80Arb_xve_impl(void* stream, const IT* query, uint8_t* kcache_data, uint8_t* vcache_data, uint8_t* mask_data, OT* outputs, uint32_t token_len, uint32_t kv_len, uint32_t kv_head, uint32_t q_head, float attn_scale)
{
...
#pragma unroll
                    ...
                    uint32_t mask_byte_offset = t * ((kv_len + 7) / 8) + token_idx / 8;
                    uint32_t mask_bit_offset = token_idx % 8;
                    uint8_t mask_int = mask_data[mask_byte_offset];
                    float mask_val = (mask_int & (1 << mask_bit_offset)) ? FP32_MIN : 0.0f;

                    // float mask_val = (float)mask_data[t * kv_len + token_idx];
                    float qkResult = sycl::ext::intel::esimd::detail::sum<float, float, HEAD_DIM>(temp) * attn_scale + mask_val;
                    if (qkResult > maxQK) {
...
void RunMhaQ80Arb_xve(void* stream, const void* query, uint8_t* kCache, uint8_t* vCache, uint8_t* mask, void* outputs, uint32_t token_len, uint32_t kv_len, uint32_t kv_head, uint32_t q_head, uint32_t head_dim, float attn_scale, int input_precision, int output_precision)
{
    if (head_dim != 128) {
        return;
    }
    ...
    if (input_precision == 0 && output_precision == 0) {
        RunVecMhaQ80Arb_xve_impl<float, float, 128>(stream, (float*)query, kCache, vCache, mask, (float*)outputs, token_len, kv_len, kv_head, q_head, attn_scale);
    } else if (input_precision == 0 && output_precision == 1) {
        RunVecMhaQ80Arb_xve_impl<float, fp16, 128>(stream, (float*)query, kCache, vCache, mask, (fp16*)outputs, token_len, kv_len, kv_head, q_head, attn_scale);
    } else if (input_precision == 1 && output_precision == 0) {
        RunVecMhaQ80Arb_xve_impl<fp16, float, 128>(stream, (fp16*)query, kCache, vCache, mask, (float*)outputs, token_len, kv_len, kv_head, q_head, attn_scale);
    } else if (input_precision == 1 && output_precision == 1) {
        RunVecMhaQ80Arb_xve_impl<fp16, fp16, 128>(stream, (fp16*)query, kCache, vCache, mask, (fp16*)outputs, token_len, kv_len, kv_head, q_head, attn_scale);
    }
}
