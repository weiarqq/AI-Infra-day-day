...
extern "C" bool runGemmXmx16_Q41Weights_L5(sycl::queue* q, uint8_t* inputs, uint8_t* weights, uint8_t* scales, uint8_t* zps, uint8_t* outputs, unsigned batch, unsigned input_len, unsigned output_len, unsigned input_precision, unsigned output_precision,  uint8_t* shuffleTt);
...
ESIMD_INLINE void gemm_q41weights_xmx16_L5(fp16 *inputs, uint8_t *weights, fp16 *scales, fp16 *zps, float *outputs, uint32_t token_len, uint32_t input_len, uint32_t output_len, float* scratch, nd_item<2>& ndi)
{
...
bool runGemmXmx16_Q41Weights_L5(sycl::queue* q, uint8_t* inputs, uint8_t* weights, uint8_t* scales, uint8_t* zps, uint8_t* outputs, unsigned token_len, unsigned input_len, unsigned output_len, unsigned input_precision, unsigned output_precision,  uint8_t* shuffleTt)
{
    const uint32_t blockWidth = 4096;
    ...
    try {
        sycl::event e = q->submit([&](handler& cgh) {
                    cgh.parallel_for(Range, [=](nd_item<2> ndi) SYCL_ESIMD_KERNEL{
                        gemm_q41weights_xmx16_L5((fp16 *)shuffleTt, (uint8_t *)weights, (fp16 *)scales, (fp16 *)zps, (float *)outputs, token_len, input_len, output_len, (float *)outputScratch, ndi);
                    });
                });
    } catch (sycl::exception const &e) {
        std::cout << "SYCL exception caught: " << e.what() << '\n';
