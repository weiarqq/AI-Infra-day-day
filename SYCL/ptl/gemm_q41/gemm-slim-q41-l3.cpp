...
extern "C" bool runSlimGemmQ41_L3(sycl::queue* q, uint8_t* inputs, uint8_t* weights, uint8_t* scales, uint8_t* zps, uint8_t* outputs, unsigned batch, unsigned input_len, unsigned output_len, unsigned input_precision, unsigned output_precision,  uint8_t* shuffleTt);

template<uint32_t batchnum>
ESIMD_INLINE void slimGemmGroup32Block128BatchN_L3(float* inputs, uint8_t* weights, fp16* scales, fp16* zps, float* outputs, uint32_t input_len, uint32_t output_len, nd_item<1>& ndi)
{
...
#pragma unroll
        for (int k = 0; k < 4; k++)
        {
            shuffledWeights.template bit_cast_view<uint16_t>().select<16, 1>(32 * k) = weightsBuffer.template bit_cast_view<uint16_t>().select<16, 2>(j * 128 + 32 * k);
            shuffledWeights.template bit_cast_view<uint16_t>().select<16, 1>(32 * k + 16) = weightsBuffer.template bit_cast_view<uint16_t>().select<16, 2>(j * 128 + 32 * k + 1);
        }
...
template<uint32_t batchnum>
ESIMD_INLINE void slimGemmGroup32Block256BatchN_L3(float* inputs, uint8_t* weights, fp16* scales, fp16* zps, float* outputs, uint32_t input_len, uint32_t output_len, nd_item<1>& ndi)
{
    slm_init(sizeof(float) * 64 * batchnum * 16);
...
bool runSlimGemmQ41_L3(sycl::queue* q, uint8_t* inputs, uint8_t* weights, uint8_t* scales, uint8_t* zps, uint8_t* outputs, unsigned batch, unsigned input_len, unsigned output_len, unsigned input_precision, unsigned output_precision,  uint8_t* shuffleTt)
{
    if (input_len % 256 != 0)
    {
        ...
        case 1:
            e = q->submit([&](handler& cgh) {
                cgh.parallel_for(Range_128, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL{
                    slimGemmGroup32Block128BatchN_L3<1>((float*)inputs, (uint8_t*)weights, (fp16*)scales, (fp16*)zps, (float*)outputs, input_len, output_len, ndi);
                });
            });
            break;
        case 2:
            e = q->submit([&](handler& cgh) {
                cgh.parallel_for(Range_128, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL{
                    slimGemmGroup32Block128BatchN_L3<2>((float*)inputs, (uint8_t*)weights, (fp16*)scales, (fp16*)zps, (float*)outputs, input_len, output_len, ndi);
                });
            });
            break;
        case 3:
            e = q->submit([&](handler& cgh) {
                cgh.parallel_for(Range_128, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL{
                    slimGemmGroup32Block128BatchN_L3<3>((float*)inputs, (uint8_t*)weights, (fp16*)scales, (fp16*)zps, (float*)outputs, input_len, output_len, ndi);
                });
            });
            break;
        case 4:
            e = q->submit([&](handler& cgh) {
                cgh.parallel_for(Range_128, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL{
                    slimGemmGroup32Block128BatchN_L3<4>((float*)inputs, (uint8_t*)weights, (fp16*)scales, (fp16*)zps, (float*)outputs, input_len, output_len, ndi);
                });
            });
            break;
        case 5:
            e = q->submit([&](handler& cgh) {
                cgh.parallel_for(Range_128, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL{
                    slimGemmGroup32Block128BatchN_L3<5>((float*)inputs, (uint8_t*)weights, (fp16*)scales, (fp16*)zps, (float*)outputs, input_len, output_len, ndi);
                });
            });
            break;
        case 6:
            e = q->submit([&](handler& cgh) {
                cgh.parallel_for(Range_128, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL{
                    slimGemmGroup32Block128BatchN_L3<6>((float*)inputs, (uint8_t*)weights, (fp16*)scales, (fp16*)zps, (float*)outputs, input_len, output_len, ndi);
                });
            });
            break;
        case 7:
            e = q->submit([&](handler& cgh) {
                cgh.parallel_for(Range_128, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL{
                    slimGemmGroup32Block128BatchN_L3<7>((float*)inputs, (uint8_t*)weights, (fp16*)scales, (fp16*)zps, (float*)outputs, input_len, output_len, ndi);
                });
            });
            break;
        case 8:
            e = q->submit([&](handler& cgh) {
                cgh.parallel_for(Range_128, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL{
                    slimGemmGroup32Block128BatchN_L3<8>((float*)inputs, (uint8_t*)weights, (fp16*)scales, (fp16*)zps, (float*)outputs, input_len, output_len, ndi);
                });
            });
            break;
        default:
