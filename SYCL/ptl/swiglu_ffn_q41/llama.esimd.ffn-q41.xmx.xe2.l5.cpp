...
extern "C" bool runQ41FfnSwiglu_L5(sycl::queue* q, uint8_t *up, uint8_t *down, uint8_t *input, uint8_t *output, uint32_t token_len, uint32_t input_len, uint32_t hidden_len, unsigned input_precision, unsigned output_precision, uint8_t *scratch_buffer);

static void shuffle_input(float *inputs, fp16 *outputs, uint32_t token_len, uint32_t input_len, nd_item<2>& ndi)
{
...
ESIMD_INLINE void swiglu_q41weights_xmx16_L5(fp16 *inputs, uint8_t *weights, fp16 *scales, fp16 *zps, fp16 *outputs, uint32_t token_len, uint32_t input_len, uint32_t hidden_len, nd_item<2>& ndi)
{
    ...
    uint32_t localOffsetWeight0 = (globalOffsetWeight + hh * 32 * input_len) / 2;
    uint32_t localOffsetWeight1 = localOffsetWeight0 + 16 * input_len / 2;
    

    uint32_t loopStep = 32;
    uint32_t loopNum = input_len / loopStep;

    uint32_t scaleOffset = h * 256 + hh * 32;
    ...
    BData0 = block_load<uint8_t, 256>(weights + localOffsetWeight0);
    BData1 = block_load<uint8_t, 256>(weights + localOffsetWeight1);

    scales_buf = block_load<fp16, 32>(scales + scaleOffset);
    zps_buf = block_load<fp16, 32>(zps + scaleOffset);


    localOffsetWeight0 += 16 * 16;
    localOffsetWeight1 += 16 * 16;
    localOffsetInput += 16 * 256;
    scaleOffset += hidden_len * 2;
...
ESIMD_INLINE void gemm_down_q41weights_xmx16_L5(fp16 *inputs, uint8_t *weights, fp16 *scales, fp16 *zps, float *outputs, uint32_t token_len, uint32_t input_len, uint32_t output_len, float* scratch, nd_item<2>& ndi)
{
    //slm_init(256 * 128 * sizeof(fp16) * 2);
    constexpr uint32_t slmOffsetScale = 0;
    constexpr uint32_t slmOffsetZps = 256 * 128 * sizeof(fp16);
...
bool runQ41FfnSwiglu_L5(sycl::queue* q, uint8_t *up, uint8_t *down, uint8_t *input, uint8_t *output, uint32_t token_len, uint32_t input_len, uint32_t hidden_len, unsigned input_precision, unsigned output_precision, uint8_t *scratch_buffer)
{
    // shuffle input
    ...
    try {
        sycl::event e = q->submit([&](handler& cgh) {
                    cgh.parallel_for(Range_up, [=](nd_item<2> ndi) SYCL_ESIMD_KERNEL{
                        swiglu_q41weights_xmx16_L5((fp16 *)shuffleTt, (uint8_t *)up, (fp16 *)scales_up, (fp16 *)zps_up, (fp16 *)gateResult, token_len, input_len, hidden_len, ndi);
                    });
                });
    } catch (sycl::exception const &e) {
        std::cout << "SYCL exception caught: " << e.what() << '\n';
    ...
    try {
        sycl::event e = q->submit([&](handler& cgh) {
                    cgh.parallel_for(Range_down, [=](nd_item<2> ndi) SYCL_ESIMD_KERNEL{
                        gemm_down_q41weights_xmx16_L5((fp16 *)gateResult, (uint8_t *)down, (fp16 *)scales_down, (fp16 *)zps_down, (float *)output, token_len, hidden_len, input_len, (float *)outputScratch, ndi);
                    });
                });
    } catch (sycl::exception const &e) {
        std::cout << "SYCL exception caught: " << e.what() << '\n';
