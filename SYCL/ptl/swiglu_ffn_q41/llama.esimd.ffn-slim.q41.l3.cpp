...
extern "C" bool runSwigluFFnVec_Q41_L3(queue *q, unsigned token_len, unsigned input_len, unsigned hidden_len, uint8_t *inputs, uint8_t *up, uint8_t *down, uint8_t *outputs, uint8_t *scratch_buffer);
...
bool runSwiglu_L3(queue & q, float *input, float *output, uint32_t token_len, uint32_t hidden_len)
{
...
bool runSwigluFFnVec_Q41_L3(queue *q, unsigned token_len, unsigned input_len, unsigned hidden_len, uint8_t *inputs, uint8_t *up, uint8_t *down, uint8_t *outputs, uint8_t *scratch_buffer)
{
    ...
    if (token_len == 1)
    {
        runLinearQ41_L3(*q, hidden_len * 2, input_len, token_len, up, inputs, s_up, z_up, upResult);
    }
    else
    {
        runSlimGemmQ41_L3(q, inputs, up, s_up, z_up, upResult, token_len, input_len, hidden_len * 2, 0, 0, scratch_buffer);
    }
    runSwiglu_L3(*q, (float *)upResult, (float *)swigluResult, token_len, hidden_len);
    if (token_len == 1)
    {
        runLinearQ41_L3(*q, input_len, hidden_len, token_len, down, swigluResult, s_down, z_down, outputs);
    }
    else
    {
        runSlimGemmQ41_L3(q, swigluResult, down, s_down, z_down, outputs, token_len, hidden_len, input_len, 0, 0, scratch_buffer);
